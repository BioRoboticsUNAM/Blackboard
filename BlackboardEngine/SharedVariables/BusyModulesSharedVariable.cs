...
{
	/// <summary>
	/// Implements a shared variable for the list of busy modules
	/// </summary>
	internal class BusyModulesSharedVariable : SharedVariable
	{
		#region Constructors
		...
		public override string ReadStringData()
		{
			...
			List<string> busyModules = new List<string>(owner.Parent.Modules.Count);
			foreach (IModule module in owner.Parent.Modules)
			{
				if (module.Busy)
					busyModules.Add(module.Name);
			}
			return "\\\"" + String.Join(" ", busyModules.ToArray()) + "\\\"";
		}
