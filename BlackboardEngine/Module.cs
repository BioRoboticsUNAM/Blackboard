...
namespace Blk.Engine
{
	...
	/// <summary>
	/// Implements an interface to access a remote module
	/// </summary>
	public class Module : IModule
	{
		...
		public Module(string name, IPAddress ipAddress, int port)
		...
		/// <param name="port">Port of the Application Module</param>
		public Module(string name, IEnumerable<IPAddress> ipAddresses, int port)
			: this(name)
		{
		...
		public Module(string name, IEnumerable<IPAddress> ipAddresses, int port, Prototype[] prototypes)
			: this(name, ipAddresses, port)
		{
		...
		*/
		...
		private void CheckAlive(ref DateTime sendNextAliveTime)
		{
			TimeSpan idleTime;
			if (!AliveCheck)
			{
			...
			*/
			...
			if (idleTime.TotalMilliseconds >= 10000)
			{
				IsAlive = false;
				if (DateTime.Now > sendNextAliveTime)
				{
					sendNextAliveTime = DateTime.Now.AddMilliseconds(10000);
					try
					{
						if (busy) Send("busy");
		...
		public static Module FromString(string s)
		{
			...
			return new Module(result.Result("${name}"), new IPAddress[]{ ip }, port, null);
